...
Bilateral trade flow forecasts use a gravity model estimated on 859 observations across 100 partner countries for FY2018–2026, combining OLS (for coefficient interpretability; in-sample R² = 0.431) and XGBoost (in-sample R² ≈ 0.94, but leave-country-out cross-validation R² ≈ 0.27 — the honest skill at predicting an unseen market, since most gravity features are time-invariant per country). News impact is assessed through a 3-layer geopolitical risk scoring architecture implementing the Steel-GPR daily index methodology of Iacoviello and Tong (2026), calibrating persistence multipliers for structural events (1.5×) versus one-off shocks (0.7×).
